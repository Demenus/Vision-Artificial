...
% haarFeatureDemo Train a decision stump based on Haarlike features.
%   This method allows to select a Haar-like feature based on two horizontal regions. 
    outSize=[30,30];
    ...
    % Select an image from the dataset to use as model
    ...
    for f=1:numFeatures,
        ...
        subplot(2,3,2);
        showFeatureVals(fval,Y,thr,polarity);title(sprintf('Feature Values. Polarity=%d',polarity));legend('Face','No-face');
        ...
        % Show the values using the sum of rectangle pixels
        subplot(2,3,5);
        stem(outClass);title(['Final prediction. Error=' sprintf('%3.1f%%',roundn(err*100,-1))]);
